...
export class DisplaySummaryComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    let data = sessionStorage.getItem('global');
    if (data) {
  ...
  populateData(res: any) {
    ...
    this.pois.forEach((ele) => {
      this.data.forEach((ele2) => {
        if (ele.id == ele2.replied_to_tweet_id) {
          ele2['reply_user'] = ele.poi_name;
          data.push(ele2);
        }
      });
    });

    let res2 = this.graphdata.multiLevelDataMapping(
      'sentiments_text',
      this.pois,
      false,
      'reply_user'
    ...
    this.barResult.forEach((ele) => {
      ...
      res2.forEach((ele2) => {
        if (ele.name == ele2.name) {
          this.replyUser.push(ele2);
        }
      });
    });

    this.replyUser.forEach((ele: any, idx: any) => {
      ele.series.forEach((item: any, idx2: any) => {
        if (item.name == 'undefined') {
          ele.series.pop(idx2);
        }
